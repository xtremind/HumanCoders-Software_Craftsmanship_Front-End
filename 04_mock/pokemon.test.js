...
describe('pokemon battle', () => {
  ...
  it.each([
  ...
  ])('should declare $firstAttacker.name first attacker given a fight between $attacker.name and $defender.name', async ({ attacker, defender, firstAttacker }) => {
    ...
    let currentBattle = await battle([{...attacker, speed: attacker.speed}, {...defender, speed: defender.speed}]);
  ...
  it.each([
  ...
  ])('should declare $loser.name loser with 0 health given a fight between $attacker.name and $defender.name', async ({ attacker, defender, loser }) => {
    axios.get.mockImplementation((url) => {
      if(url.includes("pikachu")) {
        return Promise.resolve({ data: {
          stats: [
    ...
    expect(currentBattle.loser).toEqual(expect.objectContaining({...loser, health: 0 }));
  })
  ...
  ])('should declare $winner.name winner with $health health given a fight between $attacker.name and $defender.name', async ({ attacker, defender, winner, health }) => {
    let currentBattle = await battle([{...attacker, speed: attacker.speed}, {...defender, speed: defender.speed}]);
